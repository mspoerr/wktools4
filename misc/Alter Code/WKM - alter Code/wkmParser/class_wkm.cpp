...
Wkm::Wkm() : wxEvtHandler()
{
	...
	vari[CWkmPropGrid::WKM_PROPID_INT_HIERARCHIC] = 1;
	vari[CWkmPropGrid::WKM_PROPID_INT_ORTHO] = 1;
...
void Wkm::startParser()
{
	...
	if (!dbCheck.empty())
	{
		...
		{
			// Warnung ausgeben
			//int ret = wxMessageBox("wktools needs to clean up the Database before this version can be used.\n All existing data will be dropped. Do you want to proceed?", "DB Warning", wxYES_NO);
			//if (ret == wxYES)
			//{
	...
	if (starteTool)
	{
		...
		dieDB->query("CREATE TABLE IF NOT EXISTS neighborship (n_id INTEGER PRIMARY KEY AUTOINCREMENT, dI_intf_id INTEGER, dI_dev_id INTEGER, dI_intf_id1 INTEGER, dI_dev_id1 INTEGER, flag INTEGER NULL, CONSTRAINT fk_interfaces_has_interfaces_interfaces1 FOREIGN KEY (dI_intf_id , dI_dev_id) REFERENCES devInterface (interfaces_int_id, device_dev_id) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT fk_interfaces_has_interfaces_interfaces2 FOREIGN KEY (dI_intf_id1 , dI_dev_id1) REFERENCES devInterface (interfaces_int_id , device_dev_id) ON DELETE CASCADE ON UPDATE CASCADE);");
		...
		dieDB->query("CREATE TABLE IF NOT EXISTS rControl (rc_id INTEGER PRIMARY KEY AUTOINCREMENT, n_id INTEGER, cdp_id INTEGER, dev_id INTEGER, intf_id INTEGER, neighbor_id INTEGER, vlan_id INTEGER, ipSubnet_id INTEGER, stp_status_id INTEGER, stp_id INTEGER, l3r_id INTEGER, ipphone_id INTEGER, hwinf_id INTEGER, license_id INTEGER, flash_id INTEGER, vrf_id INTEGER, timestamp TEXT, dbVer INTEGER);");
		...
		if (!dbCheck.empty())
		{
			if (vari[CWkmPropGrid::WKM_PROPID_INT_COMBINE])
			{
				...
				derCombiner = new WkmCombiner(dieDB, logAusgabe, vari[CWkmPropGrid::WKM_PROPID_INT_IMP_ES], vari[CWkmPropGrid::WKM_PROPID_INT_DNS_RESOLUTION]);
			...
			dieAusgabe->sets(vari[CWkmPropGrid::WKM_PROPID_INT_CDP], vari[CWkmPropGrid::WKM_PROPID_INT_CDPHOSTS], vari[CWkmPropGrid::WKM_PROPID_INT_UNKNOWNSTP], 
				vari[CWkmPropGrid::WKM_PROPID_INT_L2], vari[CWkmPropGrid::WKM_PROPID_INT_L3], vari[CWkmPropGrid::WKM_PROPID_INT_ORGANIC], 
...
bool Wkm::einstellungenInit()
{
	...
	std::string attInt[] = {"ParseNew", "AnalyzeNew", "CDP", "CDP_Hosts", "UnknownStp", "L2", "L3", "ORG", "HIER", "CleanupDB", "ClearDB", "Import", "revDns", "endsystems", "imp_es", "in-mem-db", "ORTHO", "istyle"};
