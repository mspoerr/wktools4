...
//					sString += "AND interfaces.channel_intf_id IS NULL ";
	...
	schreibeLog(WkLog::WkLog_ZEIT, "6620: Adjacencies - Step 5: L2 - Non Switch\n", "6620", 
		WkLog::WkLog_BLAU, WkLog::WkLog_FETT);
	sString = "SELECT intf_id,intfName,hostname,dev_id FROM interfaces ";
	sString += "INNER JOIN devInterface ON interfaces.intf_id=devInterface.interfaces_int_id ";
	sString += "INNER JOIN device ON device.dev_id=devInterface.device_dev_id ";
...
//		schreibeLog(WkLog::WkLog_NORMALTYPE, "stpKonistenzCheck - 2\r\n", "", WkLog::WkLog_ROT);
	...
	// 26.03.2013: Check deaktiviert, da Glücks-Check; 
	// Es wird nur geschaut, ob es ein freies Interface mit demselben Type gibt
...
void WkmCombiner::markAnomaly()
{
	...
	// Änderung 05.03.2013: Schritt 0.1 hinzugefügt
	// Schritt 0.1: Nachbarschaften von Nexus1k auf Non-UCS Switches löschen und Nachbarschaften von UCS<->N1k mit flag=3 markieren, damit sie vom 2-Way Check nicht erfasst werden
	sString = "SELECT intf_id FROM interfaces INNER JOIN devInterface ON devInterface.interfaces_int_id=interfaces.intf_id ";
	...
	// Änderung 04.03.: Schritt 2 deaktiviert
	...
	for(std::vector<std::vector<std::string>>::iterator it = ret.begin(); it < ret.end(); ++it)
	{
		...
		sString = "SELECT n_id FROM neighborship WHERE dI_intf_id=" + it->at(0) + " AND dI_dev_id=dI_dev_id1 AND dI_intf_id1 IN ( ";
		sString += "SELECT col FROM (SELECT dI_intf_id AS col FROM neighborship ";
		sString += "UNION ALL SELECT dI_intf_id1 FROM neighborship WHERE dI_intf_id1 > 0 ";
		sString += ") T1 GROUP BY col HAVING COUNT(*) > 1);";
		...
		if (ret1.size() > ret11.size() > 0)
		{
			// Falls dI_dev_id==dI_dev_id1 und andere Einträge, dann die dI_dev_id==dI_dev_id1 Einträge löschen
			for(std::vector<std::vector<std::string>>::iterator it1 = ret111.begin(); it1 < ret111.end(); ++it1)
			{
				sString = "DELETE FROM neighborship WHERE n_id = " + it1->at(0);
				dieDB->query(sString.c_str());
				dbgAsgbe1(sString, 2, 0, "Anomaly: 3-Way Check");
			}
	...
	// Änderung 06.03.2013: Probleme bei Nexus 1k mgmt0 Interface Nachbarschaften; Deshalb wird Schritt 5 deaktiviert
	//   Ein L3 Interface kann natürlich immer auf einem L2 Interface angehängt sein!
...
void WkmCombiner::cdpChecks()
{
	...
	sString += "INNER JOIN cdp ON cdp.cdp_id=clink.cdp_cdp_id INNER JOIN device ON device.dev_id=devInterface.device_dev_id ";
	sString += "WHERE devInterface.interfaces_int_id NOT IN (SELECT dI_intf_id AS col FROM neighborship ";
	sString += "UNION ALL SELECT dI_intf_id1 FROM neighborship) AND device.dataSource IS NULL ";
	if (!nidEmpty)
	{
