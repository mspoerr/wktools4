...
private:
	...
	void iosARPParser(					// zum Parsen der ARP Infos von IOS Boxen
		std::size_t startPos,				// Position zum Starten
		std::size_t endPos					// Position für Ende
		);
	...
	void iosVrfRouteArpParser(			// IOS VRF Routing Table und ARP Parser
		);
	void iosDot1xParser(				// IOS 802.1X Parser
		);
	void iosMabParser(					// IOS MAB Parser
		);
	void iosAuthParser(					// IOS Authentication Session Parser
		);
	void iosVlanParser(					// IOS show vlan Parser
		);
	void iosDhcpSnoopParser(			// IOS DHCP Snooping - globale Infos
		);
	void iosDhcpSnoopBindParser(		// IOS DHCP Snooping Binding Parser
		);
	void iosDevTrackingParser(			// IOS IP Device Tracking Parser
		);
	void iosMSTParser(					// IOS MST Parser
		);
	...
	void nxosFEXParser(					// zum Parsen der FEX Infos von NXOS Boxen
		);
	...
	void nxosSwitchportParser(			// NXOS show interface switchport Parser
		);
	void testCatOS(						// CatOS Tester
		);
	void catOSHWParser(					// CatOS Hardware Parser
		);
	void catosCDPParser(				// zum Parsen der CDP Infos von CatOS Boxen
		);
	...
	void pixOSFailoverParser(			// PIX License Info Parser
		);
	...
	void iosHSRPParser(					// IOS HSRP Parser
		std::size_t startPos,				// Position zum Starten
		std::size_t endPos					// Position für Ende
		);
	void nxosHSRPParser(				// NXOS HSRP Parser
		);
	void nxosOSPFParser(				// NXOS show ip ospf neigh
		);
	void nxosEIGRPParser(				// NXOS show ip eigrp neigh
		);
	void nxosBGPParser(					// NXOS show ip bgp summary Parser
		);
